...
#if 1
	double area = 0.0;
	unsigned int s = (unsigned int)p.size();
...
static void OffsetWithLoops(const TPolyPolygon &pp, TPolyPolygon &pp_new, double inwards_value)
{
	...
	for(unsigned int i = 0; i < pp.size(); i++)
	{
		...
		if(p.size() > 2)
		{
			if(reverse)
			{
				for (unsigned int j = (unsigned int)p.size() - 1; j > 1; j--)MakeLoop(p[j], p[j - 1], p[j - 2], radius);
			...
			else
			{
				...
				for (unsigned int j = 2; j < (unsigned int)p.size(); j++)MakeLoop(p[j - 2], p[j - 1], p[j], radius);
			}
	...
	else
	{
		...
		for(unsigned int i = 0; i < copy.size(); i++)
		{
			...
			TPolygon p_new;
			p_new.resize(p.size());
			unsigned int size_minus_one = (unsigned int)p.size() - 1;
			for (unsigned int j = 0; j < (unsigned int)p.size(); j++)p_new[j] = p[size_minus_one - j];
			pp_new[i] = p_new;
		}
	}
}
...
static void MakePolyPoly( const CArea& area, TPolyPolygon &pp, bool reverse = true ){
	...
	for(std::list<CCurve>::const_iterator It = area.m_curves.begin(); It != area.m_curves.end(); It++)
	{
		...
		if(reverse)
		{
			unsigned int i = (unsigned int)pts_for_AddVertex.size() - 1;// clipper wants them the opposite way to CArea
...
CArea CArea::UniteCurves(std::list<CCurve> &curves)
{
	...
	TPolyPolygon solution;
	c.Execute(ctUnion, solution, ClipperLib::pftNonZero, ClipperLib::pftNonZero);
	CArea area;
