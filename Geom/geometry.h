...
//		~Point3d();
	...
	public:
		...
		double normalise(){
			double m = magnitude(); if (m < 1.0e-09) { dx = dy = dz = 0; return 0; } dx /= m; dy /= m; dz /= m;		// normalise & returns magnitude
			return m;
		}
...
//	Kurve splineUsingBiarc(CLine& cl0, CLine& cl1, std::vector<pts>);
	...
	public:
		...
		const Vector3d& getV0(void) { return v0; }
		const Vector3d& getV1(void) { return v1; }
